import 'package:flutter/material.dart';

class AppColors {
  static const background = Colors.white;
  static const accentColor = Color(0xFF50E3C2);
  static const textOverWhite = Color(0xFF000000);
  static const textGrey = Color(0xFFB5B5B5);
  static const secondary = Color(0xFFFFCE1F);
  static const textWhiteImg = Color(0xFFFFFFFF);
  static const GreyLightButton = Color(0xFFF1F1F1);

  static const Gradient gradient =
      LinearGradient(colors: [Color(0xFFEFAFA9), Color(0xFF5B3DDE)]);
}
